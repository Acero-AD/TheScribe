...
- [x] 1.1 Decide the `<app>` subdomain string; reserve `<app>` and `<app>-api` DNS records on Cloudflare
- [x] 1.2 Create a Hetzner VPS (smallest shared-vCPU tier), add a dedicated deploy SSH key, note the public IP
- [x] 1.3 Create a Neon project with a single Postgres database; capture the pooled `DATABASE_URL`
- [x] 1.4 Create a Cloudflare Pages project for the frontend; create a scoped Cloudflare API token + note account/project IDs
- [x] 1.5 Create a Resend account, verify the sending domain, and generate an SMTP/API credential
- [x] 1.6 Create a GHCR access token (or use the Actions `GITHUB_TOKEN`) for image push/pull

## 2. Backend — collapse to a single database

- [x] 2.1 Take a Neon branch/snapshot as a rollback point before config changes
...
- [x] 6.1 Run `kamal setup` from a workstation; verify the box boots the image and TLS is valid
- [x] 6.2 Point Cloudflare `<app>-api` at the box (proxied, SSL mode `Full`)
- [x] 6.3 Smoke-test: request a magic link (email delivered) and verify an Active Storage upload round-trips
...
- [x] 7.3 Do a first `wrangler pages deploy` of the build; point Cloudflare `<app>` at the Pages project
- [x] 7.4 Smoke-test: load the app at `<app>`, confirm it reaches the API and login works end-to-end

## 8. CI — independent deploy on green master

- [x] 8.1 Add GitHub Actions secrets: GHCR token, deploy SSH key, `RAILS_MASTER_KEY`, `DATABASE_URL`, Resend credential, `CLOUDFLARE_API_TOKEN` + account/project IDs, `VITE_BACKEND_URL`
...
- [x] 9.1 Backend-only change on `master` deploys only the backend
- [x] 9.2 Frontend-only change on `master` deploys only the frontend
- [x] 9.3 A change touching both deploys both; a failing pipeline blocks its deploy; a PR deploys nothing
- [x] 9.4 Confirm `kamal rollback` and Cloudflare Pages rollback both work as recovery paths
