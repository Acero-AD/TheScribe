...
- [x] 7.1 Validate the change with `openspec validate add-ci-pipelines --strict`
...
- [ ] 7.3 Open a PR touching only `frontend/`; confirm frontend jobs run, backend jobs are skipped, and `ci-success` passes — requires push + PR
- [ ] 7.4 Open a PR touching only `backend/`; confirm backend jobs (incl. Postgres-backed tests) run, frontend jobs are skipped, and `ci-success` passes — requires push + PR
- [ ] 7.5 In GitHub branch protection for `master`, require only the `ci-success` status check (document this manual step in the PR description) — manual GitHub setting
