# weekly-publishing Specification

## Purpose
Per-week record of whether the user published this week, plus a Today-screen card that captures the toggle. Owns the `WeekLog` model and the week-level read/write API, including week-start derivation from the user's timezone and `week_starts_on`. Establishes the weekly publish check-in as a composition surface on `/` for later weekly widgets (e.g. streaks).
