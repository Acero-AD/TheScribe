...
## Purpose
Cadence-aware computation of the user's writing and publishing streaks, surfaced inline with the existing daily-log and week-log endpoints and rendered inside the existing check-in cards on `/`. Owns the algorithms, the `publishing_cadence` interpretation, and the streak-display behavior on the Today screen. Streaks are computed on demand from existing `DailyLog` / `WeekLog` rows — no denormalized cache at v1.
